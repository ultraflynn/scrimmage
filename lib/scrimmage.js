...
Scrimmage.prototype.listen = function(port) {
	...
	async.series([
		...
		function(callback) {
			primeServersCache(self, callback);
		},
		function(callback) {
...
function primePlayerlistsCache(self, callback) {
	// TODO playerlist for everything
	callback(null);
}

function primePlayersCache(self, callback) {
	// TODO player for each player
	callback(null);
}

function primeDogtagsCache(self, callback) {
	// TODO dogtag for each player
	callback(null);
}

function primeServersCache(self, callback) {
	// TODO server for each server
	callback(null);
}
...
Scrimmage.prototype.playerlist = function(players, res) {
	...
	if (players === undefined || players.length === 0) {
		...
		self.api.playerlist(players, {}, function(err, data) {
			if (err) {
...
Scrimmage.prototype.playerupdate = function(player, type, res) {
	...
	if (player === undefined) {
		...
		self.api.playerupdate(player, type, function(data) {
			if (err) {
				res.end(createError(err));
			} else {
				var content = JSON.stringify(data);
...
Scrimmage.prototype.playerlookup = function(player, res) {
	...
	if (player === undefined) {
		...
		self.api.playerlookup(player, function(data) {
			if (err) {
				res.end(createError(err));
			} else {
				var content = JSON.stringify(data);
...
Scrimmage.prototype.setupkey = function(clientident, name, res) {
	...
	if (clientident === undefined) {
		...
		self.api.setupkey(clientident, name, function(data) {
			if (err) {
				res.end(createError(err));
			} else {
				var content = JSON.stringify(data);
...
Scrimmage.prototype.getkey = function(clientident, res) {
	...
	if (clientident === undefined) {
		...
		self.api.getkey(clientident, name, function(data) {
			if (err) {
				res.end(createError(err));
			} else {
				var content = JSON.stringify(data);
